        ...
        <div className='flex flex-col items-center my-3 space-y-2 pb-12'>
            ...
            <div className='space-x-4 space-y-2'>
              {project.ferramentas.map(ferramenta => (
                <span className='bg-secondary rounded-full text-gray-900 text-primary text-sm px-4 py-1 inline-block'>{ferramenta}</span>
              ))}
            </div>
        </div>
        <button className='absolute mx-auto w-48 left-0 right-0 bottom-4 bg-primary text-white px-8 py-2 rounded-full transition transform duration-300 hover:scale-105'>Ver Detalhes</button>
    </div>
  )
}
